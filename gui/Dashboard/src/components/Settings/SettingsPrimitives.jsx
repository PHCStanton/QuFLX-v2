...
export const SettingsSection = ({ title, children, defaultOpen = true }) => {
  ...
  return (
    <div className="mb-4 rounded-lg border border-border-primary bg-card-bg/50 overflow-hidden shadow-sm dark:shadow-none">
      <button
        onClick={() => setIsOpen(!isOpen)}
        className="flex w-full items-center justify-between p-3 text-left hover:bg-section-bg/30 transition-colors"
      >
...
export const SliderInput = ({ value, min, max, step = 1, onChange, unit = '' }) => (
  <div className="flex items-center gap-3 w-48">
    <input
      ...
      className="flex-1 h-1.5 bg-gray-300 dark:bg-gray-700 rounded-lg appearance-none cursor-pointer accent-accent-green"
    />
...
export const RadioGroup = ({ value, options, onChange }) => (
  <div className="flex bg-section-bg/50 rounded p-1 border border-border-primary shadow-inner dark:shadow-none">
    {options.map((opt) => (
      <button
        key={opt.value}
        onClick={() => onChange(opt.value)}
        className={`px-3 py-1 rounded text-[10px] font-bold transition-colors ${
          value === opt.value ? 'bg-accent-green text-white dark:text-black shadow-sm' : 'text-text-secondary hover:text-text-primary'
        }`}
      >
        {opt.label}
      </button>
