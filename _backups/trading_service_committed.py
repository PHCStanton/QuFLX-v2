"""
trading_service.py ΓÇö Live Trading Service (Singleton)
...
# Lazy imports ΓÇö only attempted after path is set
...
class TradingService:
    ...
    def _load_config(self) -> None:
        ...
        with self._config_lock:
            try:
                ...
                logger.warning("Failed to load trading config: %s ΓÇö using defaults", exc)
                self._config = dict(_DEFAULT_CONFIG)
    ...
    def _connect_sync(self, ssid: str, demo: bool) -> Dict[str, Any]:
        """Blocking connect ΓÇö runs inside executor."""
        ...
        try:
            # ΓÜá∩╕Å  Python 3.10+: asyncio.get_event_loop() raises RuntimeError in
            ...
            # ΓÜá∩╕Å  The PocketOption API ignores the demo= argument.
    ...
    def _execute_trade_sync(
        ...
        """Blocking trade execution ΓÇö runs inside executor."""
        # Guard: connection
        ...
        if amount < min_amt or amount > max_amt:
            return {
                "success": False,
                "error": f"Amount ${amount} outside allowed range ${min_amt}ΓÇô${max_amt}",
            }
    ...
    async def switch_mode(self, demo: bool) -> Dict[str, Any]:
        """
        Switch Demo Γåö Real by reconnecting with the SSID for the target mode.
