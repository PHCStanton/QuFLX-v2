...
> [!IMPORTANT]
> **`httpx` Not in `requirements.txt`**: The `httpx` library is used by `AIService` but is NOT listed in `requirements.txt`. This should be added to prevent deployment failures.
...
**Reviewed Files:**
- `service.py` — Fresh `httpx.AsyncClient` created per-request
- `test_ai_service.py` — Existing tests (5 test cases)
- `routes/ai.py` — AI endpoint integration
...
#### [MODIFY] `service.py`
Add persistent client in `__init__`:
```python
...
1. **Connection Health Monitoring**: Add logging for connection pool stats to detect leaks.
   ```python
   ...
   ```
2. **Circuit Breaker Pattern**: Consider adding a circuit breaker for AI calls to prevent cascading failures when Grok API is degraded.
...
### Current State Analysis
**Reviewed Files:**
- `otc_alert_dispatch.py` — 710 lines, main dispatcher
...
#### [MODIFY] `otc_alert_dispatch.py`

**Phase 1: Scanner Improvements**
1. **Increase candle count** (configurable):
   ```python
   CANDLE_COUNT = int(os.getenv("ALERT_CANDLE_COUNT", "100"))
   ```
2. **Add EMA Cross Detection (9/21)**:
   ```python
...
4. **Add Support/Resistance (fractal pivots)**:
   ...
   ```
5. **Add Confidence Score (confluence-based)**:
   ```python
   @dataclass
   class ScanResult:
...
1. **Confidence threshold filter**:
   ...
   if ai_verdict.confidence < MIN_CONFIDENCE:
       ...
       return
   ```
2. **AI confirmation toggle (env var)**:
   ```python
...
### Manual Verification
1. **Connection Pooling Test (Task 1):**
...
| Date | Task | Sub-task |
| :--- | :--- | :--- |
| **2026-02-07** | Task 1 | Add `httpx` and `tenacity` to requirements |
| **2026-02-07** | Task 1 | Refactor `AIService` client |
| **2026-02-07** | Task 1 | Add lifespan hooks |
| **2026-02-07** | Task 1 | Update tests |
| **2026-02-07** | Task 2 | Enhance `MarketScanner` |
| **2026-02-07** | Task 2 | Improve AI parsing |
| **2026-02-07** | Task 2 | Add confidence filtering |
| **2026-02-07** | Task 2 | Add AI toggle |
