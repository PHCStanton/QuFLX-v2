"""Netstat-based Chrome Session Starter

This script starts a Chrome session with remote debugging enabled using built-in Windows
commands instead of external dependencies like psutil.
...
def check_if_chrome_running():
    """Check if Chrome with remote debugging is already running using netstat."""
    try:
        # Use netstat to check if port 9222 is LISTENING
        result = subprocess.run(['netstat', '-a', '-n'], capture_output=True, text=True, timeout=10)
        return ':9222 ' in result.stdout and 'LISTENING' in result.stdout
    except subprocess.TimeoutExpired:
        print("Warning: netstat command timed out")
        return False
...
if __name__ == "__main__":
    print("Netstat-based Chrome Session Starter")
    print("=" * 40)
